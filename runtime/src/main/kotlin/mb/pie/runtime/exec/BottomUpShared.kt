 ...
 */
fun StoreReadTxn.directlyAffectedTaskKeys(changedFiles: Collection<ResourceKey>, resourceSystems: ResourceSystems, logger: Logger): HashSet<TaskKey> {
  ...
  for(changedFile in changedFiles) {
    ...
    for(key in requirees) {
      ...
      if(!fileReqs(key).filter { it.key == changedFile }.all { it.isConsistent(resourceSystems) }) {
        affected.add(key)
      }
    }
    ...
    if(generator != null) {
      ...
      if(!fileGens(generator).filter { it.key == changedFile }.all { it.isConsistent(resourceSystems) }) {
