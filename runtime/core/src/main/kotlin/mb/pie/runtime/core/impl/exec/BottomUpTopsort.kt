...
open class BottomUpTopsortExec(
  ...
  fun requireBottomUpInitial(changedPaths: List<PPath>, cancel: Cancelled) {
    ...
    val directlyAffected = store.readTxn().use { txn -> directlyAffectedApps(changedPaths, txn, logger) }

    val comparator = Comparator<UFuncApp> { app1, app2 ->
      when {
        app1 == app2 -> 0
    ...
    val queue = PriorityQueue<UFuncApp>(comparator)
    queue.addAll(directlyAffected)

    requireBottomUp(queue, directlyAffected, cancel)
  }
  ...
  internal open fun requireBottomUp(queue: PriorityQueue<UFuncApp>, set: MutableSet<UFuncApp>, cancel: Cancelled) {
