...
interface DirtyFlagger {
  fun flag(changedPaths: List<PPath>, txn: StoreWriteTxn)
}
...
class DirtyFlaggerImpl @Inject constructor(
  ...
  override fun flag(changedPaths: List<PPath>, txn: StoreWriteTxn) {
  ...
  private fun pathIsConsistent(path: PPath, funcApp: UFuncApp, txn: StoreReadTxn, checkerGenFunc: (PPath, UExecRes) -> ConsistencyChecker?): Boolean {
    ...
    if(result != null) {
      ...
      // Should not happen, log error and assume change
      logger.error("Could not find result for function application ${funcApp.toShortString(200)} that requires $path")
      return false
    }
    return true
  }
