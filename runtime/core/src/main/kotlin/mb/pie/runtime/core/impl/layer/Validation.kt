...
class ValidationBuildLayer @Inject constructor(logger: Logger) : BuildLayer {
  ...
  override fun <I : In, O : Out> validate(app: BuildApp<I, O>, result: BuildRes<I, O>, build: Build) {
    build.storeReadTxn().use { txn ->
      for((path, _) in result.gens) {
        val generatedBy = txn.generatedBy(path)
        if(generatedBy != null) {
          val builder = build.getBuilder<I, O>(result.builderId)
        ...
        if(requiredBy != null && !stack.contains(requiredBy)) {
          ...
          if(requiredByResult != null && !hasBuildReq(requiredByResult, app, txn, build)) {
      ...
      for((path, _) in result.reqs.filterIsInstance<PathReq>()) {
        ...
        if(generator != null && !hasBuildReq(result, generator, txn, build)) {
          error("""Hidden dependency. Cause:
            |build
            |  ${result.desc}
            |requires path
  ...
  private fun hasBuildReq(requiree: UBuildRes, generator: UBuildApp, txn: BuildStoreReadTxn, build: Build): Boolean {
    ...
    while(!toCheckQueue.isEmpty()) {
      ...
      val builder = build.getAnyBuilder(toCheck.builderId)
      if(toCheck.requires(generator, build)) {
        return true
      }
      ...
      for(reqRequest in reqRequests) {
        val reqResult = txn.produces(reqRequest) ?: kotlin.error("Cannot get result for app $reqRequest")
  ...
  private fun <I : In, O : Out> validateInput(app: BuildApp<I, O>) {
  ...
  private fun <I : In, O : Out> validateOutput(result: BuildRes<I, O>) {
  ...
  private fun <T : Serializable> deserialize(bytes: ByteArray): T {
    ByteArrayInputStream(bytes).use { inputStream -> ObjectInputStream(inputStream).use { objectInputStream -> return objectInputStream.readObject() as T } }
  }
