  ...
  override fun <I : In, O : Out> validate(app: FuncApp<I, O>, data: FuncAppData<O>, funcs: Funcs, txn: StoreReadTxn) {
    for((path, _) in data.pathReqs) {
      ...
      when {
        ...
        !hasCallReq(app, generator, funcs, txn) -> {
    ...
    for((path, _) in data.pathGens) {
      ...
      when {
        ...
        @Suppress("UNCHECKED_CAST") !funcs.getFunc<I, O>(app.id).mayOverlap(app.input, generator.input as I) -> {
      ...
      for(requiredBy in requiredByApps) {
        when {
          ...
          !hasCallReq(requiredBy, app, funcs, txn) -> {
            error("""Hidden dependency. Cause:
              |path
              |  $path
              |was generated by
