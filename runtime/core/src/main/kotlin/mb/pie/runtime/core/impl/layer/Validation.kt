...
class ValidationLayer @Inject constructor(logger: Logger) : Layer {
  ...
  override fun <I : In, O : Out> validate(app: FuncApp<I, O>, result: ExecRes<I, O>, funcs: Funcs, txn: StoreReadTxn) {
    for((path, _) in result.gens) {
      ...
      for(requiredBy in requiredByApps) {
        ...
        if(!stack.contains(requiredBy)) {
          val requiredByResult = txn.resultOf(requiredBy)
          if(requiredByResult != null && !hasBuildReq(requiredByResult, app, txn, funcs)) {
    ...
    for((path, _) in result.reqs.filterIsInstance<PathReq>()) {
      ...
      if(generator != null && !hasBuildReq(result, generator, txn, funcs)) {
        error("""Hidden dependency. Cause:
            |build
            |  ${result.desc}
            |requires path
  ...
  private fun hasBuildReq(requiree: UExecRes, generator: UFuncApp, txn: StoreReadTxn, funcs: Funcs): Boolean {
    ...
    while(!toCheckQueue.isEmpty()) {
      ...
      if(toCheck.calls(generator, funcs)) {
        return true
      }
      val reqRequests = toCheck.reqs.filterIsInstance<UCallReq>().map { it.callee }
      val reqResults = mutableListOf<UExecRes>()
      for(reqRequest in reqRequests) {
        val reqResult = txn.resultOf(reqRequest) ?: kotlin.error("Cannot get result for app $reqRequest")
