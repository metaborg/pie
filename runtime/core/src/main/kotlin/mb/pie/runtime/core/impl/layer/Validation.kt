...
class ValidationLayer @Inject constructor(logger: Logger) : Layer {
  ...
  override fun <I : In, O : Out> validatePreWrite(app: FuncApp<I, O>, data: FuncAppData<O>, funcs: Funcs, txn: StoreReadTxn) {
    for((path, _) in data.pathReqs) {
      val generator = txn.generatorOf(path)
      when {
        generator == null -> { // No generator for `path`.
        }
        app == generator -> { // Required `path` generated by itself (`app`).
        }
        !hasCallReq(app, generator, txn) -> {
          // `path` is required by `app`, and `path` is generated by `generator`, thus `app` must (transitively) require `generator`.
  ...
  override fun <I : In, O : Out> validatePostWrite(app: FuncApp<I, O>, data: FuncAppData<O>, funcs: Funcs, txn: StoreReadTxn) {
    for((path, _) in data.pathGens) {
      val generatedBy = txn.generatorOf(path)
      if(generatedBy != null) {
        val func = funcs.getFunc<I, O>(app.id)
        // CHANGED: builders may describe if certain different build applications may generate the same file.
        @Suppress("UNCHECKED_CAST")
        if(!func.mayOverlap(app.input, generatedBy.input as I)) {
      ...
      // CHANGED: 'path' is generated by 'result', and path is required by 'requireesOf', thus 'requireesOf' must (transitively) require 'app'.
      ...
      for(requiredBy in requiredByApps) {
        ...
        if(!stack.contains(requiredBy)) {
          if(!hasCallReq(requiredBy, app, txn)) {
            error("""Hidden dependency. Cause:
              |path
              |  $path
              |was generated by
  ...
  private fun hasCallReq(requiree: UFuncApp, generator: UFuncApp, txn: StoreReadTxn): Boolean {
    // TODO: more efficient implementation for figuring out if an app transitively depends on another app?
    ...
    while(!toCheckQueue.isEmpty()) {
      ...
      val calls = txn.callReqs(toCheck).map { it.callee }.toHashSet()
      if(calls.contains(generator)) {
        return true
      }
      toCheckQueue.addAll(calls)
    }
    return false
  }
