 ...
 *
 * <p>
 * Implements a DAG that can be modified (vertices &amp; edges added and removed), is guaranteed to remain acyclic, and
 * provides fast topological order iteration. An attempt to add an edge which would induce a cycle throws an {@link
 * IllegalArgumentException}.
 *
 * <p>
 * This is done using a dynamic topological sort which is based on the algorithm described in "David J. Pearce &amp;
 * Paul H. J. Kelly. A dynamic topological sort algorithm for directed acyclic graphs. Journal of Experimental
 * Algorithmics, 11, 2007." (see
 ...
 * implementation differs from the algorithm specified in the above paper in some ways, perhaps most notably in that the
 * topological ordering is stored by default using two hash maps, which will have some effects on the runtime, but also
 * allow for vertex addition and removal. This storage mechanism can be adjusted by subclasses.
 *
 * <p>
 * The complexity of adding a new edge in the graph depends on the number of edges incident to the "affected region",
 * and should in general be faster than recomputing the whole topological ordering from scratch. For details about the
 * complexity parameters and running times, see the previously mentioned paper.
 *
 * <p>
 * This class makes no claims to thread safety, and concurrent usage from multiple threads will produce undefined
 * results.
 *
 ...
 * @author Peter Giles
 */
class DirectedAcyclicGraph<V, E> extends AbstractBaseGraph<V, E> implements Iterable<V> {
     ...
     * @param vertexSupplier         the vertex supplier
     * @param edgeSupplier           the edge supplier
     * @param visitedStrategyFactory the visited strategy factory. Subclasses can change this implementation to adjust
     *                               the performance tradeoffs.
     * @param topoOrderMap           the topological order map. For performance reasons, subclasses can change the way
     *                               this class stores the topological order.
     * @param weighted               if true the graph will be weighted, otherwise not
     */
     ...
     *
     * <p>
     * The complexity of adding a new edge in the graph depends on the number of edges incident to the "affected
     * region", and should in general be faster than recomputing the whole topological ordering from scratch.
     *
     ...
     * @param visited       a simple data structure that lets us know if we already visited a node with a given topo
     *                      index
    ...
    protected interface TopoOrderMap<V> extends Serializable {
         ...
         *
         * @param vertex the vertex
         * @return the index that the vertex is at, or null if the vertex isn't in the topological ordering
         */
         ...
         *
         * @param vertex the vertex
         * @return the index that the vertex was at, or null if the vertex wasn't in the topological ordering
         */
    ...
    protected interface VisitedStrategy {
         ...
         * @throws UnsupportedOperationException if the implementation doesn't support (or doesn't need) clearance. For
         *                                       example, if the factory creates a new instance every time, it is a
         *                                       waste of cycles to reset the state after the search of the Affected
         *                                       Region is done, so an UnsupportedOperationException *should* be
         *                                       thrown.
         */
    ...
    /**
     * An implementation of the topological order map which for performance and flexibility uses an ArrayList for
     * topological index to vertex mapping, and a HashMap for vertex to topological index mapping.
     *
     * @author Peter Giles
     */
    protected class TopoVertexMap implements TopoOrderMap<V> {
        ...
        /**
         * We translate the topological index to an ArrayList index. We have to do this because topological indices can
         * be negative, and we want to do it because we can make better use of space by only needing an ArrayList of
         * size |AR|.
         *
     ...
     *
     * <p>
     * This implementation is close to the performance of {@link VisitedArrayListImpl}, with 1/8 the memory usage.
     *
     * @author John V. Sichi
     */
     ...
     *
     * <p>
     * This implementation seems to offer the best performance in most cases. It grows the internal ArrayList as needed
     * to be as large as |AR|, so it will be more memory intensive than the HashSet implementation, and unlike the Array
     * implementation, it will hold on to that memory (it expands, but never contracts).
     *
     * @author Peter Giles
     */
     ...
     *
     * <p>
     * This implementation doesn't seem to perform as well, though I can imagine circumstances where it should shine
     * (lots and lots of vertices). It also should have the lowest memory footprint as it only uses storage for indices
     ...
     * reallocation of the underlying array for every topology reorder that is required.
     *
     * @author Peter Giles
     */
